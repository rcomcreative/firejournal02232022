...
protocol ShiftStartCVCellDelegate: AnyObject {
    func shiftStartBTapped()
}

class ShiftStartCVCell: UICollectionViewCell {
    ...
    weak var delegate: ShiftStartCVCellDelegate? = nil
...
"""
        ...
        if shiftImage != nil {
            ...
            shiftB.tintColor = UIColor(named: "FJGreenColor")
        }
    ...
    @objc func theButtonTapped(_ sender: UIButton) {
        DispatchQueue.main.async {
            self.nc.post(name:Notification.Name(rawValue: FJkSTARTSHIFT_FROM_DETAIL), object: nil, userInfo: nil )
        }
    }
