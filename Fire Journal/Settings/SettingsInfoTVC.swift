...
class SettingsInfoTVC: UITableViewController {
    ...
    weak var delegate:SettingsInfoDelegate? = nil
    ...
    var textViewHeightForCell: CGFloat = 820
    ...
    override func viewDidLoad() {
        ...
        case .compact:
            ...
            _ = UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(updateTheData(_:)))
...
//            navigationItem.setRightBarButtonItems([button2], animated: true)
    ...
    private func configureTheText() {
        switch settingType {
        case .cloud?:
            subject = "Membership"
            descriptionText = "Membership is the master back office companion to Fire Journal for iOS devices. When you subscribe to Fire Journal Cloud, you add a full suite of additional power to your operational activities. Fire Journal Cloud does everything Fire Journal does, and adds in reporting, tracking, additional forms, export of forms to government approved formats, to PDF, and other capabilities. Every firefighter using Fire Journal should consider adding Fire Journal Cloud to their operational resources."
            bodyText = cloudText()
        case .terms?:
    ...
    override func viewWillLayoutSubviews() {
        descriptionTV.setContentOffset(.zero, animated: true)
    }
    ...
    override func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        default:
            let color = ButtonsForFJ092018.gradient11Color
            headerV.colorV.backgroundColor = color
        }
        headerV.subjectL.text = subject
        headerV.subjectL.textColor = UIColor.white
        switch settingType {
        case .cloud:
            headerV.infoB.isEnabled = false
            headerV.infoB.isHidden = true
            headerV.infoB.alpha = 0.0
        case .privacy:
            headerV.infoB.isEnabled = false
            headerV.infoB.isHidden = true
            headerV.infoB.alpha = 0.0
        case .terms:
            ...
            headerV.infoB.isHidden = true
            headerV.infoB.alpha = 0.0
            headerV.subjectL.adjustsFontSizeToFitWidth = true
        default: break
        }
...
//        }
    ...
    override func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if textViewHeightForCell == 820 {
            textViewHeightForCell = getSizeForTextView()
        }
        return textViewHeightForCell
    }
    
    
}

extension SettingsInfoTVC {
    
    private func getSizeForTextView() ->CGFloat {
        if bodyText != "" {
            let textView = UITextView()
            textView.frame = CGRect(x: 0, y: 0, width: self.tableView.frame.width, height: textViewHeightForCell)
            let size = CGSize(width: textView.frame.width, height: .infinity)
            textView.text = bodyText
            let estimatedSize = textView.sizeThatFits(size)
            switch settingType {
            case .cloud:
            textViewHeightForCell = estimatedSize.height + 1020
            case .privacy:
            textViewHeightForCell = estimatedSize.height + 4420
            case .terms:
            textViewHeightForCell = estimatedSize.height + 3420
            default: break
            }
        }
        return textViewHeightForCell
...
extension SettingsInfoTVC: SettingsInfoHeaderVDelegate {
