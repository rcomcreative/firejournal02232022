...
class FJICS214Loader: FJOperation {
    ...
    func theCounter()->Int {
        ...
        do {
            ...
            fjICS214FormA = try bkgrdContext.fetch(fetchRequest) as!  [ICS214Form]
            return count
    ...
    func chooseNewOrUpdate(withCompletion completion: () -> Void ) {
        for record in ckRecordA {
            if let guid:String = record["ics214Guid"] {
                let result = fjICS214FormA.filter { $0.ics214Guid == guid }
                if result.isEmpty {
                    newICS214FromCloud(record: record)
                } else {
                    fjICS214 = result.last
                    updateICS214FromCloud(fjICS214Record: record, fjuICS214: fjICS214)
                }
            }
        }
        completion()
    ...
    func updateICS214FromCloud(fjICS214Record: CKRecord, fjuICS214: ICS214Form ) {
