    //
    //  FJUpdateIncidentAddressToFCLocationOperation.swift
    //  Fire Journal
    //
...
class FJUpdateIncidentAddressToFCLocationOperation: FJOperation {
    ...
    var updatedIncidentCKRecordsA = [CKRecord]()
    var count: Int = 0
    var stop:Bool = false
    var recordID:String = ""
    var counter = 0
    ...
    override func main() {
        ...
        getTheIncidents()
        let result = theIncidentsA.filter { $0.incidentLocationSC != nil }
        if !result.isEmpty {
            createFCLocationForIncident(incidents: result) {
                saveToCD()
                updateCloudKitWithIncidentCKRecords {
                    self.updateCloudKitWithFCLocationCKRecords() {
                        DispatchQueue.main.async {
                            self.nc.post(name: .fireJournalUpdateJournalLocationToFCLocation, object: nil)
    ...
    func updateCloudKitWithIncidentCKRecords(withCompletion completion: @escaping() -> Void) {
        let modifyCKOperation = CKModifyRecordsOperation.init(recordsToSave: updatedIncidentCKRecordsA, recordIDsToDelete: nil)
    ...
    func createFCLocationForIncident(incidents: [Incident], withCompletion completion: () -> Void ) {
        for incident in incidents {
            let theLocation = FCLocation(context: bkgrdContext)
            theLocation.guid = UUID.init()
            theLocation.incidentGuid = incident.fjpIncGuidForReference
            incident.theLocation = theLocation
            ...
            if let location = incident.incidentLocationSC {
                ...
                do {
                    ...
                    incident.locationAvailable = true
                    
                    if incident.incidentAddressDetails != nil {
                        if let incidentAddress = incident.incidentAddressDetails {
                            if let streetNumber = incidentAddress.streetNumber {
                                theLocation.streetNumber = streetNumber
                            }
                            if let streetName = incidentAddress.streetHighway {
                                theLocation.streetName = streetName
                            }
                            if let city = incidentAddress.city {
                                theLocation.city = city
                            }
                            if let state = incidentAddress.incidentState {
                                theLocation.state = state
                            }
                            if let zip = incidentAddress.zip {
                                theLocation.zip = zip
                            }
                        }
                    } else {
            ...
            if let ckr = incident.fjIncidentCKR {
                
                guard let  archivedData = ckr as? Data else { return }
                do {
                    let unarchiver = try NSKeyedUnarchiver.init(forReadingFrom: archivedData)
                    if let incidentCKRecord = CKRecord(coder: unarchiver) {
                    incidentCKRecord["locationAvailable"] = 1
                    let coder = NSKeyedArchiver(requiringSecureCoding: true)
                    incidentCKRecord.encodeSystemFields(with: coder)
                    let data = coder.encodedData
                    incident.fjIncidentCKR = data as NSObject
                    updatedIncidentCKRecordsA.append(incidentCKRecord)
                    }
                } catch {
    ...
    fileprivate func saveToCD() {
        do {
            ...
            DispatchQueue.main.async {
                self.nc.post(name:NSNotification.Name.NSManagedObjectContextDidSave,object: self.bkgrdContext ,userInfo:["info":"FJUpdateIncidentAddressToFCLocationOperation here"])
            }
