//
//  NewICS214ActivityLogOperation.swift
//  Fire Journal
//
...
class NewICS214ActivityLogOperation: FJOperation {
    ...
    override func main() {
        ...
        operation = "NewICS214ActivityLogOperation"
        ...
        if objectID == fjICS214ActivityLog.objectID {
            let ics214 = fjICS214ActivityLog?.ics214ActivityInfo as! Set<ICS214Form>
            ...
            for ics in ics214 {
                ...
                do {
                    ...
                    ckRecord = fjICS214ActivityLog.newICS214ActivityLogToTheCloud(ckRecordID: ckRecordID)
                } catch {
    ...
    fileprivate func saveToCD() {
        do {
            ...
            DispatchQueue.main.async {
                self.nc.post(name:NSNotification.Name.NSManagedObjectContextDidSave,object:self.bkgrdContext,userInfo:["info":"New ICS214 Activity Log Operation"])
            }
            DispatchQueue.main.async {
                print("NewICS214ActivityLogSendToCloudOperation has run and now if finished")
                self.executing(false)
                self.finish(true)
                self.nc.removeObserver(self, name: NSNotification.Name.NSManagedObjectContextDidSave, object: nil)
            }
    ...
    private func theCount(guid: String)->Int {
        ...
        do {
            ...
            fjICS214ActivityLogA = try context.fetch(fetchRequest) as! [ICS214ActivityLog]
            fjICS214ActivityLog = fjICS214ActivityLogA.last
            return count
