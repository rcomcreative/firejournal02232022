...
protocol NewStoreBetaVCDelegate: AnyObject {
...
class NewStoreBetaVC: UIViewController {
    
    //    MARK:  - OBJECTS -
    @IBOutlet weak var monthlyB: UIButton!
    @IBOutlet weak var quarterlyB: UIButton!
    @IBOutlet weak var annualB: UIButton!
    @IBOutlet weak var loginB: UIButton!
    @IBOutlet weak var restorePurchaseB: UIButton!
    
    //    MARK: -PROPERTIES -
    weak var delegate: NewStoreBetaVCDelegate? = nil
...
//        annually = defaults.string(forKey: FJkSUBSCRIPTION3) ?? ""
    ...
    @IBAction func monthlyBTapped(_ sender: Any) {
        fjSKProduct = nil
        defaults.set(false, forKey: "subscriptionBought")
        defaults.synchronize()
        purchaseMonthProduct()
    }
    ...
    @IBAction func restorePurchaseBTapped(_ sender: Any) {
        SKPaymentQueue.default().add(self)
    ...
    func purchaseQuarterlyProduct() {
        ...
        if available {
            if !alertUp {
                self.dismiss(animated: true, completion: nil)
                if(self.canMakePurchases()){
                    if fjQuarterlySKProduct != nil {
                    let pay = SKPayment(product: fjQuarterlySKProduct)
                    SKPaymentQueue.default().add(pay)
                    SKPaymentQueue.default()
                    productID = fjQuarterlySKProduct.productIdentifier
                    alertUp = true
                    }
               }
            }
...
extension NewStoreBetaVC: SKProductsRequestDelegate, SKPaymentTransactionObserver {
...
//          fjSKProduct = product
//           if(self.canMakePurchases()){
//               payment = SKPayment(product: fjSKProduct)
//               SKPaymentQueue.default().add(payment)
////               SKPaymentQueue.default()
//
//               print("PRODUCT TO PURCHASE: \(fjSKProduct.productIdentifier)")
//               productID = fjSKProduct.productIdentifier
//           } else {
//               print("can't make a purchase right now")
//           }
//    }
