...
class JournalTagsCVCell: UICollectionViewCell {
    
    let theBackgroundView = UIView()
    let tagL = UILabel()
    var theTag: JournalTags!
...
extension JournalTagsCVCell {
    
    func configure(tag: JournalTags) {
        self.theTag = tag
        if let name = self.theTag.journalTag {
            tagL.text = name
        }
