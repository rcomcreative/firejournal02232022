//
//  ARCrossFormModifiedToCloudOperation.swift
//  FJ ARC Plus
//
...
class ARCrossFormModifiedToCloudOperation: FJOperation {
    ...
    override func main() {
        ...
        if let ckr = arcrossForm.arcFormCKR {
            ...
            do {
                ...
                let modifiedCKRecord = arcrossForm.modifyARCrossFormForCloud(ckRecord: ckRecord)
                ...
                modifyCKOperation.perRecordCompletionBlock = { record, error in
                    ...
                    print(record)
                    self.arcrossForm.arcBackup = true
                    let coder = NSKeyedArchiver(requiringSecureCoding: true)
                    record.encodeSystemFields(with: coder)
                    let data = coder.encodedData
                    self.arcrossForm.arcFormCKR = data as NSObject
                }
    ...
    fileprivate func saveToCD() {
        do {
            ...
            DispatchQueue.main.async {
                self.nc.post(name:NSNotification.Name.NSManagedObjectContextDidSave,object:self.bkgrdContext,userInfo:["info":"ARCrossFormModifiedToCloudOperation"])
            }
            DispatchQueue.main.async {
                print("ARCrossFormModifiedToCloudOperation has run and now if finished")
                self.executing(false)
                self.finish(true)
                self.nc.removeObserver(self, name: NSNotification.Name.NSManagedObjectContextDidSave, object: nil)
            }
