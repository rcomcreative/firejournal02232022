...
class CloudKitManager: NSObject {
        ...
        for zoneId in zoneIDs {
            ...
            changesOperation.recordChangedBlock = { [weak self] record in
                ...
                if let entity = record["theEntity"] as? String {
                    print("here is the entity \(entity)")
                    switch entity {
    ...
    @objc func zoneRecordsCalled(notification: Notification)->Void {
        ...
        if let userInfo = notification.userInfo as! [String: Any]?
        {
                ...
                self.recordEntity = userInfo["recordEntity"] as? TheEntities ?? TheEntities.fjUser
                
                    //            pendingOperations.nfirsIncidentTypeQueue.isSuspended = true
                
                switch self.recordEntity {
                case TheEntities.fjIncident:
                    self.cloudType = CloudTypes.incidents
                    if !self.incidentRs.isEmpty {
                        self.incidentSyncOperation.incidentSyncQueue.isSuspended = true
                        var fjIncidents = Array(NSOrderedSet(array: self.incidentRs)) as! [CKRecord]
                        
                        let fjIncidentLoader = FJIncidentLoader.init( self.context , ckArray: fjIncidents)
                        self.incidentSyncOperation.incidentSyncQueue.addOperation(fjIncidentLoader)
                        self.incidentSyncOperation.incidentSyncQueue.isSuspended = false
                        self.incidentRs.removeAll()
                ...
                case TheEntities.fjJournal:
                    self.cloudType = CloudTypes.journals
                    if !self.journalRs.isEmpty {
                        self.journalSyncOperation.journalSyncQueue.isSuspended = true
                        var fjJournals = Array(NSOrderedSet(array: self.journalRs)) as! [CKRecord]
                        print("here is journalRs count: \(self.journalRs.count)")
                        
                        let fjJournalLoader = FJJournalLoader.init( self.context , ckArray: fjJournals)
                        self.journalSyncOperation.journalSyncQueue.addOperation(fjJournalLoader)
                        self.journalSyncOperation.journalSyncQueue.isSuspended = false
                        self.journalRs.removeAll()
                ...
                case TheEntities.fjUser:
                    self.cloudType = CloudTypes.fjuser
                    if !self.fjuserRs.isEmpty {
                        self.userSyncOperation.userSyncQueue.isSuspended = false
                        var fjUsers = Array(NSOrderedSet(array: self.fjuserRs)) as! [CKRecord]
                        
                        let fjUserLoader = FJIUserSyncOperation.init( self.context , ckArray: fjUsers)
                        self.userSyncOperation.userSyncQueue.addOperation(fjUserLoader)
                        self.userSyncOperation.userSyncQueue.isSuspended = true
                        self.fjuserRs.removeAll()
                        fjUsers.removeAll()
                ...
                case TheEntities.fjUserTime:
                    self.cloudType = CloudTypes.userTime
                    if !self.userTimRs.isEmpty {
                        self.userTimeSyncOperation.userTimeSyncQueue.isSuspended = true
                        var fjUserTimes = Array(NSOrderedSet(array: self.userTimRs)) as! [CKRecord]
                        
                        let fjUserTimeLoader = FJUserTimeSyncOperation.init( self.context ,ckArray: fjUserTimes)
                        self.userTimeSyncOperation.userTimeSyncQueue.addOperation(fjUserTimeLoader)
                        self.userTimeSyncOperation.userTimeSyncQueue.isSuspended = false
                        self.userTimRs.removeAll()
                ...
                case TheEntities.fjAttendee:
                    self.cloudType = CloudTypes.userAttendee
                    if !self.userAttendeesRs.isEmpty {
                        self.userAttendeeSyncOperation.userAttendeeSyncQueue.isSuspended = true
                        var fjUserAttendees = Array(NSOrderedSet(array: self.userAttendeesRs)) as! [CKRecord]
                        
                        let fjUserAtteendeesLoader = FJUserAttendeesFromCloudSyncingOperation.init( self.context , ckArray: fjUserAttendees)
                        self.userAttendeeSyncOperation.userAttendeeSyncQueue.addOperation(fjUserAtteendeesLoader)
                        self.userAttendeeSyncOperation.userAttendeeSyncQueue.isSuspended = false
                        self.fjUserAttendeeRs.removeAll()
                ...
                case TheEntities.fjStatus:
                    ...
                    if self.fjStatusRs.isEmpty {
                        ...
                        var statusFromCloud = FJStatusSyncOperation.init( self.context , ckArray: fjStatus)
                ...
                case TheEntities.fjICS214:
                    self.cloudType = CloudTypes.ics214
                    if !self.ics214Rs.isEmpty {
                        self.ics214SyncOperation.ics214SyncQueue.isSuspended = true
                        var fjics214s = Array(NSOrderedSet(array: self.ics214Rs)) as! [CKRecord]
                        
                        let fjICS214Loader = FJICS214Loader.init( self.context , ckArray: fjics214s)
                        self.ics214SyncOperation.ics214SyncQueue.addOperation(fjICS214Loader)
                        self.ics214SyncOperation.ics214SyncQueue.isSuspended = false
                        self.ics214Rs.removeAll()
                        fjics214s.removeAll()
                ...
                case TheEntities.fjArcForm:
                    self.cloudType = CloudTypes.arcForm
                    if !self.arcCrossFormRs.isEmpty {
                        self.arcFormSyncOperation.arcFormSyncQueue.isSuspended = true
                        var fjARCFormss = Array(NSOrderedSet(array: self.arcCrossFormRs)) as! [CKRecord]
                        
                        let fjARCFormLoader = FJARCCrossFormLoader.init( self.context , ckArray: fjARCFormss)
                        self.arcFormSyncOperation.arcFormSyncQueue.addOperation(fjARCFormLoader)
                        self.arcFormSyncOperation.arcFormSyncQueue.isSuspended = false
                        self.arcCrossFormRs.removeAll()
                ...
                case TheEntities.fjICS214ActivityLog:
                    self.cloudType = CloudTypes.icsActivityLog
                    if !self.fjICS214ActivityLogRs.isEmpty {
                        self.ics214ActivityLogSyncOperation.ics214ActivityLogSyncQueue.isSuspended = true
                        var fjICS214ALs = Array(NSOrderedSet(array: self.fjICS214ActivityLogRs)) as! [CKRecord]
                        
                        let ics214ALogLoader = FJICS214ActivityLogOperation.init( self.context , ckArray: fjICS214ALs)
                        self.ics214ActivityLogSyncOperation.ics214ActivityLogSyncQueue.addOperation(ics214ALogLoader)
                        self.ics214ActivityLogSyncOperation.ics214ActivityLogSyncQueue.isSuspended = false
                        self.fjICS214ActivityLogRs.removeAll()
                ...
                case TheEntities.fjICS214Personnel:
                    self.cloudType = CloudTypes.ics214Personel
                    if !self.fjICS214PersonnelRs.isEmpty {
                        self.ics214PersonalSyncOperation.ics214PersonalSyncQueue.isSuspended = true
                        var fjICS214Ps = Array(NSOrderedSet(array: self.fjICS214PersonnelRs)) as! [CKRecord]
                        
                        let ics214Personnel = FJICS214PersonnelOperation.init( self.context , ckArray: fjICS214Ps)
                        self.ics214PersonalSyncOperation.ics214PersonalSyncQueue.addOperation(ics214Personnel)
                        self.ics214PersonalSyncOperation.ics214PersonalSyncQueue.isSuspended = false
                        self.fjICS214PersonnelRs.removeAll()
                ...
                case TheEntities.fjUserFDResource:
                    self.cloudType = CloudTypes.userFDResources
                    self.firstRun = self.userDefaults.bool(forKey: FJkFIRSTRUNFORDATAFROMCLOUDKIT)
                    if self.fjUserFDResourcesRs.isEmpty {
                        DispatchQueue.main.async {
                            self.nc.post(name:Notification.Name(rawValue:FJkCKZoneRecordsCALLED),
                                         object: nil,
                        ...
                        self.fdResourcesSyncOperation.fdResourcesSyncQueue.isSuspended = true
                        var fjUserFDResources = Array(NSOrderedSet(array: self.fjUserFDResourcesRs)) as! [CKRecord]
                        
                        let ufdResources = FJUserFDResourcesSyncOperation.init( self.context , ckArray: fjUserFDResources, firstRun: self.firstRun)
                        self.fdResourcesSyncOperation.fdResourcesSyncQueue.addOperation(ufdResources)
                        self.fdResourcesSyncOperation.fdResourcesSyncQueue.isSuspended = false
                        self.fjUserFDResourcesRs.removeAll()
                        fjUserFDResources.removeAll()
                        
                        self.userDefaults.set(false, forKey: FJkFIRSTRUNFORDATAFROMCLOUDKIT)
                        self.firstRun = false
                    }
                ...
                default:
                    print("here is an entity we didn't expect \(self.recordEntity)")
                }
    ...
    func createNotificationObservers() {
        ...
        nc.addObserver(self, selector:#selector(modifiedProjectSendToCloud(nc:)),name: .fireJournalNewProjectCreatedSendToCloud, object: nil)
        ...
        if let userInfo = nc.userInfo as! [String: Any]? {
            if let objectID = userInfo["objectID"] as? NSManagedObjectID {
                DispatchQueue.global(qos: .background).async {
                    ...
                    let _ = self.theProjectProvider.theProjectToCloud(self.theProjectContext, objectID) { theProject in
                        print("here is the location that was saved \(theProject)")
                    }
            ...
            if let objectID = userInfo["objectID"] as? NSManagedObjectID {
                DispatchQueue.global(qos: .background).async {
                    ...
                    let _ = self.theProjectProvider.theProjectCrewToCloud(self.theProjectContext, objectID) { theProjectCrew in
                        print("here is the location that was saved \(theProjectCrew)")
                    }
    ...
    @objc private func projectTagSendToCloud(nc: Notification) {
        if let userInfo = nc.userInfo as! [String: Any]? {
            if let objectID = userInfo["objectID"] as? NSManagedObjectID {
                DispatchQueue.global(qos: .background).async {
                    self.theProjectContext = self.theProjectProvider.persistentContainer.viewContext
                    let _ = self.theProjectProvider.theProjectTagToCloud(self.theProjectContext, objectID) { theProjectTag in
                        print("here is the location that was saved \(theProjectTag)")
                    }
